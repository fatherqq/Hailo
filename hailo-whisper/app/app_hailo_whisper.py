"""Main app for Hailo Whisper (command-line / microphone demo).

This mirrors the original Hailo example entry point. The Home Assistant addon
uses ``app.api_server`` instead; this module is provided for local testing and
development with a microphone.
"""
...
def get_args():
    ...
    parser.add_argument("--multi-process-service", action="store_true",
...
def get_hef_path(model_variant: str, hw_arch: str, component: str) -> str:
    ...
    if not os.path.exists(hef_path):
        raise FileNotFoundError(
            ...
            f"If not done yet, please run: python3 -m app.download_resources "
            f"--hw-arch {hw_arch} --variant {model_variant}"
        )
    return hef_path
...
def main():
    ...
    whisper_hailo = HailoWhisperPipeline(
        encoder_path, decoder_path, variant,
    ...
    while True:
        if args.reuse_audio:
            ...
            if user_input.lower() == "q":
                break
            record_audio(args.duration, audio_path=audio_path)
