...
class HailoWhisperPipeline:
    ...
    def __init__(self, encoder_model_path: str, decoder_model_path: str, variant="tiny", host="arm64", multi_process_service=False):
        ...
        # Token embedding (loaded from the persistent model dir)
    ...
    def _inference_loop(self):
        ...
        if self.multi_process_service:
            ...
            # Allow overriding the shared group id via env (default: HAILO_SHARED)
            params.group_id = os.environ.get("HAILO_VDEVICE_GROUP_ID", "HAILO_SHARED")
    ...
    def get_model_input_audio_length(self):
        """Get the expected input audio length (seconds) for the encoder."""
    ...
    def get_transcription(self, timeout=120):
        """Retrieve the next transcription result (with a safety timeout)."""
